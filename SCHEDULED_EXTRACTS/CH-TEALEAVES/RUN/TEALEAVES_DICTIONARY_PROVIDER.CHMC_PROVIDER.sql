...
,ISNULL((SELECT DProvQ.Response 
					FROM [CH_MTLIVE].[dbo].[DMisProviderQuery] DProvQ
...
FROM			[CH_MTLIVE].[dbo].[DMisProvider]													AS DMISPROV
LEFT OUTER JOIN [CH_MTLIVE].[dbo].[DMisAbsService]													AS DMISAS								ON DMISPROV.SpecialtyAbsServiceID=DMISAS.AbsServiceID
LEFT OUTER JOIN [CH_MTLIVE].[dbo].[DMisUsers]														AS DMisU								ON DMISPROV.ProviderID=DMisU.ProviderID
LEFT OUTER JOIN [CH_MTLIVE].[dbo].[DMisProviderGroup]												AS DMPG									ON DMISPROV.ProviderGroupID=DMPG.ProviderGroupID	
LEFT OUTER JOIN [CH_MTLIVE].[dbo].[PpEmployees]													AS PPE									ON PPE.UserID=DMisU.UserID
LEFT OUTER JOIN [CH_MTLIVE].[dbo].[PpEmployeePayroll]												AS PPEPay								ON PPE.EmployeeID=PPEPay.EmployeeID

--LEFT OUTER JOIN [CH_MTLIVE].[dbo].[DMisProviderQuery]												AS DMISPROVQ							ON DMISPROV.ProviderID=DMISPROVQ.ProviderID												

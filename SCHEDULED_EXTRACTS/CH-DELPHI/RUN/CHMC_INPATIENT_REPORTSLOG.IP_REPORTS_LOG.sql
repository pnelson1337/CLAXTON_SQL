...
LEFT JOIN	[CH_MTLIVE].[dbo].[BarVisitProviders]																AS ATTEND	 ON AV.VisitID=ATTEND.VisitID AND ATTEND.VisitProviderTypeID='Attending'
LEFT JOIN	[CH_MTLIVE].[dbo].[DMisDischargeDisposition]														AS DISPO	 ON BV.DischargeDispositionID=DISPO.DispositionID
LEFT JOIN	[CH_MTLIVE].[dbo].[DMisProvider]																	AS PROVNAME	 ON ATTEND.ProviderID=PROVNAME.ProviderID
LEFT JOIN	[CH_MTLIVE].[dbo].[PcmDcmts]																		AS DCMTS	 ON AV.VisitID=DCMTS.VisitID
LEFT JOIN	[CH_MTLIVE].[dbo].[DMisUsers]																		AS MISUSER	 ON DCMTS.UserID=MISUSER.UserID
LEFT JOIN	[CH_MTLIVE].[dbo].[DMisProvider]																	AS DCMTSPROV ON MISUSER.ProviderID=DCMTSPROV.ProviderID
...
AND DCMTSPROV.ProviderGroupID='HOSP'

...
,[Livedb].[dbo].[StripNonNumerics](                     LEFT(BV.HomePhone,CHARINDEX(')',BV.HomePhone))                         )										AS 'PhoneAreaCityCode'    
,[Livedb].[dbo].[StripNonNumerics]( CASE WHEN BV.HomePhone NOT LIKE '(___)%' THEN BV.HomePhone ELSE SUBSTRING(BV.HomePhone,6,(LEN(BV.HomePhone)-5)) END)				AS 'PhoneLocalNumber'
...
LEFT JOIN				[Livedb].[dbo].[BarVisits]							AS BV			ON AV.[AccountNumber]=BV.[AccountNumber]
LEFT JOIN				[Livedb].[dbo].[AdmVisitPatientEmail]				AS EMAIL		ON AV.VisitID=EMAIL.VisitID
--ADM QUERIES--
LEFT JOIN				[Livedb].[dbo].[AdmVisitQueries]					AS INST			ON INST.VisitID = BV.VisitID AND INST.QueryID = 'INST'
LEFT JOIN				[Livedb].[dbo].[AdmVisitQueries]					AS LANG			ON LANG.VisitID = BV.VisitID AND LANG.QueryID = 'ADM.LANG'
LEFT JOIN				[Livedb].[dbo].[AdmVisitQueries]					AS MHUEVAL		ON MHUEVAL.VisitID = BV.VisitID AND MHUEVAL.QueryID = 'MHU EVAL'
LEFT JOIN				[Livedb].[dbo].[AdmVisitQueries]					AS ETH			ON ETH.VisitID = BV.VisitID AND ETH.QueryID = 'ETHNICITY'
--DICTIONARIES--
LEFT JOIN				[Livedb].[dbo].[DMisAdmitSource]					AS ADMITSRC		ON BV.AdmitSourceID=ADMITSRC.AdmitSourceID
LEFT JOIN				[Livedb].[dbo].[DMisDischargeDisposition]			AS DISDISPO		ON BV.DischargeDispositionID=DISDISPO.DispositionID
LEFT JOIN				[Livedb].[dbo].[DMisInsurance]						AS INS			ON BV.PrimaryInsuranceID=INS.InsuranceID AND INS.Active='Y'
--DIAGNOSIS CODES--
LEFT JOIN				[Livedb].[dbo].[AbsDrgDiagnoses]					AS ONEDIAG		ON BV.VisitID=ONEDIAG.VisitID AND ONEDIAG.DiagnosisSeqID='1'
LEFT JOIN				[Livedb].[dbo].[AbsDrgDiagnoses]					AS TWODIAG		ON BV.VisitID=TWODIAG.VisitID AND TWODIAG.DiagnosisSeqID='2'
LEFT JOIN				[Livedb].[dbo].[AbsDrgDiagnoses]					AS THREEDIAG	ON BV.VisitID=THREEDIAG.VisitID AND THREEDIAG.DiagnosisSeqID='3'
--CPT CODES--
LEFT JOIN				[Livedb].[dbo].[BarCptCodes]						AS CPTONE		ON BV.VisitID=CPTONE.VisitID AND CPTONE.CptSeqID='1'
LEFT JOIN				[Livedb].[dbo].[BarCptCodes]						AS CPTWO		ON BV.VisitID=CPTWO.VisitID AND CPTWO.CptSeqID='2'
LEFT JOIN				[Livedb].[dbo].[BarCptCodes]						AS CPTHREE		ON BV.VisitID=CPTHREE.VisitID AND CPTHREE.CptSeqID='3'
--PROVIDERS--
...
LEFT JOIN				[Livedb].[dbo].[DMisProvider]						AS PROV			ON EMERG.ProviderID=PROV.ProviderID AND PROV.Active='Y'										




WHERE CAST(BV.ServiceDateTime AS DATE) BETWEEN @StartDate AND @EndDate 

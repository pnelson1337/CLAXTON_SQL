...
LEFT JOIN	[CH_MTLIVE].[dbo].[AdmPatDischargePlanData]																AS DISPO		ON BV.VisitID=DISPO.VisitID
LEFT JOIN	[CH_MTLIVE].[dbo].[DMisDischargeDisposition]															AS DISDISPO		ON DISPO.DispositionID=DISDISPO.DispositionID AND DISDISPO.Active='Y'

LEFT JOIN	(SELECT * FROM(
SELECT ROW_NUMBER() OVER (PARTITION BY RXMORD.VisitID ORDER BY RXMORD.OrderID)										AS OrderSeq
 ,RXMORD.*
 FROM		[CH_MTLIVE].[dbo].[RxmOrds]																				AS RXMORD
 LEFT JOIN	[CH_MTLIVE].[dbo].[RxmOrdDetails]																			AS RXMOD		ON RXMORD.OrderID=RXMOD.OrderID
 LEFT JOIN	[CH_MTLIVE].[dbo].[DMisOutLocations]																		AS OL			ON RXMOD.ResourceOutsideLocationID=OL.OutsideLocationID 
...
LEFT JOIN	[CH_MTLIVE].[dbo].[RxmOrdDetails]																			AS RXMOD		ON RXMORD.OrderID=RXMOD.OrderID
LEFT JOIN	[CH_MTLIVE].[dbo].[DMisOutLocations]																		AS OL			ON RXMOD.ResourceOutsideLocationID=OL.OutsideLocationID
LEFT JOIN	[CH_MTLIVE].[dbo].[RxmOrdReferProviders]																	AS RXMORDRP		ON RXMORD.OrderID=RXMORDRP.OrderID
LEFT JOIN	[CH_MTLIVE].[dbo].[RxmOrdReferGroups]																		AS RXMORDG		ON RXMORD.OrderID=RXMORDG.OrderID
LEFT JOIN	[CH_MTLIVE].[dbo].[DMisUsers]																				AS DMU			ON RXMORD.EnteredByUserID=DMU.UserID
--ATTENDED APPOINTMENT--
LEFT JOIN	[CH_MTLIVE].[dbo].[AdmPatDischargePlanFormQueries]															AS FU11			ON AV.VisitID=FU11.VisitID AND FU11.QueryID='MHUFU1.1' AND RXMORD.OrderSeq = '1'
--REASON (IF NO)--
LEFT JOIN	[CH_MTLIVE].[dbo].[AdmPatDischargePlanFormQueries]															AS FU12			ON AV.VisitID=FU12.VisitID AND FU12.QueryID='MHUFU1.2' AND RXMORD.OrderSeq = '1'
--RESCHED DATE?--
LEFT JOIN	[CH_MTLIVE].[dbo].[AdmPatDischargePlanFormQueries]															AS FU13			ON AV.VisitID=FU13.VisitID AND FU13.QueryID='MHUFU1.3' AND RXMORD.OrderSeq = '1'
--RESCHED ATTEND?--
LEFT JOIN	[CH_MTLIVE].[dbo].[AdmPatDischargePlanFormQueries]															AS FU14			ON AV.VisitID=FU14.VisitID AND FU14.QueryID='MHUFU1.4' AND RXMORD.OrderSeq = '1'
